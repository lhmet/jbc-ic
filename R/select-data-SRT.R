...
select_data_SRT <- function(
                            data, neighbors, aws_code, m, obs_unit = c("date", "int")) {
  neigh_codes <- stations_neighbors(neighbors, aws_id = aws_code, M = m)

  data4srt <- dplyr::select(data, dplyr::one_of(c(obs_unit, neigh_codes))) %>%
