...
exdata <- data_frame(site = "A800", 
                     date = seq(as.POSIXct("2000-01-01 00:00:00"),
                                length.out = 20,
                                by = "hour"), 
                     tar = c(20.6, NA, 22, 22, 22, 23.1, 14.7, 17.7, 25.4, 19.6, 23.4, 24, 
24, 21.3, NA, NA, 26, 26, 26, 26))
DT::datatable(exdata, options =list(pageLength=20,dom = 't'))
```
...
DT::datatable(exdata_qc, options = list(pageLength=20,dom = 't'))
```
...
meta_qc <- mutate(exdata_qc, 
                  id_suspect = crv_detect(x = tar, thresh = -99, min.steps = 2, numerate = TRUE)) %>%
  filter(!is.na(id_suspect)) %>%
  group_by(id_suspect) %>%
