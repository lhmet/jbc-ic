...
Para aplicar o teste de regressão espacial (SRT) serão usados:
 - séries diárias da variável de interesse (temperatura do ar)
 - tabela de distâncias entre as EMAs

Dados.

```{r}
tair_dly <- rio::import("../output/s08_data_sel.rds") %>%
  as_tibble %>%
...
Distância de cada EMA às demais da rede de estações meteorológicas.
...
# Parâmetros necessários para aplicação do SRT

```{r}
REF <- "A803" # EMA de referência
# REF = "A812" # EMA de referência
M <- 10 # Número de EMAs selecionadas dentro de uma certa distância da EMA de interesse
...
date_0 <- "2016-09-22" # Dia de referência utilizado nos dados horários
# date_0 = "2016-08-22" # Dia utilizado nos dados horários
...
# Seleção das EMAs vizinhas à EMA de interesse

Aqui começa o corpo da função. 

O primeiro passo é filtrar as distâncias de acordo com a EMA de referência (0).
...
if (TYPE == "hourly") {
  ...
  ema_data_if <-
    ...
    filter(
      as.Date(date) >= (as.Date(date_0) - hourly_INTERVAL) & 
      as.Date(date) <= (as.Date(date_0) + hourly_INTERVAL))
  }
```
...
if (TYPE == "daily") {
  ema_data_if <-
    ...
    group_by(
      ...
      date = as.Date(date)) %>%
    summarise(
      value = mean(value, na.rm = TRUE)) %>% #group_by(site) %>% tally() %>%
    ungroup()
  } # sem NA nos dados
```
